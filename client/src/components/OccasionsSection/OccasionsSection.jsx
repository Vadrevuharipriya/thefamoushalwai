...
function OccasionCard({ occasion, large = false, eager = false }) {
  return (
    <Link
      ...
      className="occasions-section__bento-card aspect-[4/3]"
    >
      <img
        src={occasion.image}
        alt={occasion.name}
      ...
      <div className="occasions-section__bento-card-content">
        <h3 className={`font-heading font-black text-white leading-tight ${large ? 'text-lg' : 'text-xs'}`}>
          {occasion.name}
        </h3>
        <div className="flex items-center justify-between mt-1">
          <span className={`font-body text-brand-gold font-semibold ${large ? 'text-xs' : 'text-[10px]'}`}>
            Starting {occasion.price}/plate
          </span>
          <div className="w-5 h-5 rounded-full bg-brand-red flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex-shrink-0">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
          {featured.map((occasion) => (
            <div key={occasion.id} className="md:col-span-2 md:row-span-2">
